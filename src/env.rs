...
pub struct Env {
    ...
    builtin_ops: HashMap<String, Rc<dyn Fn(SyntaxTree) -> SyntaxTree>>,
...
impl Env {
    ...
    fn make_builtin_op<F: 'static>(&mut self, name: &str, func: F)
    where
        F: Fn(SyntaxTree) -> SyntaxTree,
    {
        self.builtin_ops.insert(name.to_owned(), Rc::new(func));
    }
